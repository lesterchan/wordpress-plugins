...
def verify(slug, name, prefix, port, root):
    ...
    if ci and tpl_ci:
        ...
        # The block build is a step rather than a job, so it is cut by name
        # instead of by _drop, which anchors on the next job. Everything from
        # the step's comment block to the step after it goes.
        def _drop_block_build(text):
            return re.sub(
                r"\n      # Blocks register from build/.*?(?=\n      # The runner already has Docker)",
                "", text, flags=re.S)

        # A plugin earns the build step by having block sources to build. Eleven
        # of the nineteen never will, and requiring an npm ci of them on all six
        # matrix rows would be minutes of CI spent installing a toolchain with
        # nothing to compile.
        has_blocks = os.path.isdir(os.path.join(root, "src"))
    ...
    if pkg_raw:
        ...
        if scripts is not None:
            ...
            # Block sources are the only thing here with a compile step, so the
            # two wp-scripts entries are owed exactly when src/ exists and are
            # an error when it does not: a build script in a plugin with nothing
            # to build is a command that fails the first time anybody runs it.
            if os.path.isdir(os.path.join(root, "src")):
                expected |= {"build", "start"}
    ...
    if tpl_phpcs and phpcs:
        # Excluding build/ is owed only where a build/ can exist. Carrying the
        # exclusion in a plugin with no src/ would be a rule about a directory
        # that never appears, which is how a shared file accumulates dead
        # clauses nobody dares remove.
        has_blocks = os.path.isdir(os.path.join(root, "src"))
        ...
        def _norm(t):
            ...
            if not has_blocks:
                t = re.sub(r"\n\n\t<!--\s*Generated by bin/build\..*?"
                           r"<exclude-pattern>/build/\*</exclude-pattern>",
                           "", t, flags=re.S)
